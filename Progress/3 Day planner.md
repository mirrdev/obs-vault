...
#### План (внимание ответственность)</span>
...
То есть `map` строит барьер абстракции, удаляя нас от деталей того, как реализовано то, с чем мы работаем. Поэтому за `map` может скрываться всё, что угодно. Деревья, множества, какие-то сложные вещи, которые не так просто обходить, но можем об этом уже даже не задумываться
